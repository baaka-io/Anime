...
const queryParamsToSearchAnimeOptions = (queryParams: any) => 
    Object
        ...
        .reduce((acc: object, keys: any) => ({...acc, [keys.target]: queryParams[keys.source]}), {})

const router = Router()
	.get("/video", validateQuery(VideoQuery) , async (req: ValidatedRequest<VideoQuery>, res) => {
    ...
    .get("/search", arrayQueryParams(["genre"]), validateQuery(SearchQuery), async (req, res) => {
